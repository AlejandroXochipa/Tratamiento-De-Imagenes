...
public class ImageFFT {  //Fourier Fast Transform
    ...
    public static final double gaussianHighPassFunction(
            int n, double radius, double r) {
        try {
            double r_Cuadrado = ((-1) * (Math.pow(r, 2.0)));
            double radio_Cuadrado = ((2) * (Math.pow(radius, 2.0)));
            double p = Math.exp((r_Cuadrado / radio_Cuadrado));
            return 1.0 - p;
        } catch (ArithmeticException e) {
            return 0.0;
        }
    ...
    public void frequencyEmphashysFilter(int n, double radius)
            throws FFTException {
        ...
        for (int v = 0; v < height; ++v) {
            ...
            for (int u = 0; u < width; ++u, ++i) {
                su = shift(u, u2) - u2;
                r = Math.sqrt(su * su + sv * sv) / rmax;
                //mag = gaussianHighPassFunction(n, radius, r)*data[i].getMagnitude(); o 0.75
                mag = 0.5 + (2.0 * gaussianHighPassFunction(n, radius, r));
                mag = mag * data[i].getMagnitude();
                //mag = mag* transfor();
                data[i].setPolar(mag, data[i].getPhase());
            }
        }
    ...
    //Equalizacion del histograma de la imagen 
    ...
    //Equalizado2
    public BufferedImage equalize2(BufferedImage bi) {
...
// transform image using sum histogram as a Lookup table
    ...
    public double conversion(int grados) {
        double radianes = 0;
        if (grados == 460) {
            return radianes = (double) ((double) (100) / 360);
        } else {
            return radianes = (double) ((double) (grados) / 360);
        }
    }
